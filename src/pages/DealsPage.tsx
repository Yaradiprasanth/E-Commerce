...
export const DealsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setError(null);
        setLoading(true);
        // Backend search is title-based; for a "deals" page we fetch all products and sort by price.
  ...
  const deals = useMemo(() => {
    return [...allProducts].sort((a, b) => a.price - b.price).slice(0, 20);
  }, [allProducts]);

  const heroImage = deals[0]?.image;
      ...
      {loading ? (
        ...
        <>
          <div className="relative mb-5 overflow-hidden rounded-3xl bg-white p-0 shadow-sm">
            {heroImage ? (
              <img src={heroImage} alt="Deals hero" className="h-auto w-full object-cover" />
            ) : (
          ...
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-900">Deal Picks</h2>
            <p className="text-xs font-semibold text-slate-500">{deals.length} items</p>
          </div>
          {error && (
          ...
          {!error && deals.length === 0 && (
            <p className="rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-600">
              No products found for deals right now. Try again later.
            </p>
          )}
